...
public class WeightHandler {
	...
	public static float getArmorWeightClient(ItemStack piece) {
		...
		if (!piece.isEmpty()) {
			if (DataMaps.Client.armor.containsKey(BuiltInRegistries.ITEM.getKey(piece.getItem())) && !SEnchantmentHelper.hasWeightless(piece)) {
				int i = SEnchantmentHelper.getFeatherweightModifier(piece);
				//Reduces the total weight of that armor piece by 18% for each level
				totalWeight += DataMaps.Client.armor.get(BuiltInRegistries.ITEM.getKey(piece.getItem())).getWeightModifier() * (1 - i*0.18);
			}
		}
		return totalWeight;
	}
	...
	public static float getArmorWeight(ItemStack piece) {
		float totalWeight = 0.0F;
		if (!piece.isEmpty()) {
			if (DataMaps.Server.armor.containsKey(BuiltInRegistries.ITEM.getKey(piece.getItem())) && !SEnchantmentHelper.hasWeightless(piece)) {
				int i = SEnchantmentHelper.getFeatherweightModifier(piece);
				//Reduces the total weight of that armor piece by 18% for each level
				totalWeight += DataMaps.Server.armor.get(BuiltInRegistries.ITEM.getKey(piece.getItem())).getWeightModifier() * (1 - i*0.18);
			}
		}
		return totalWeight;
	}
