...
public class SleepData extends SurviveData {
	...
	public void addTiredEffect(ServerPlayer player) {
		if (tirednessAmplifier() >= 0 && !player.hasEffect(SMobEffects.ENERGIZED.holder().value())) {
			player.addEffect(new MobEffectInstance(SMobEffects.TIREDNESS.holder().value(), 200, Math.min(tirednessAmplifier(), Survive.CONFIG.tiredTimeStacks), false, false, true));
		}
	}
